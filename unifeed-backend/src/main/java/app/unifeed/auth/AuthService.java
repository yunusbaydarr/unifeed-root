...
public class AuthService {
    private final JdbcTemplate jdbc; private final PasswordEncoder passwords; private final StringRedisTemplate redis;
    ...
    public AuthService(JdbcTemplate jdbc, PasswordEncoder passwords, StringRedisTemplate redis, JwtService jwt,
        ...
        this.jdbc=jdbc; this.passwords=passwords; this.redis=redis; this.jwt=jwt; this.refresh=refresh; this.kafka=kafka;
    ...
    public UUID register(String email, String password, String displayName, Locale locale) {
        ...
        String otp="%06d".formatted(random.nextInt(1_000_000)); redis.opsForValue().set("otp:"+normalized,otp,Duration.ofMinutes(3));
    ...
    public AuthTokens verifyOtp(String email,String otp) {
        String normalized=email.trim().toLowerCase(Locale.ROOT); String stored=redis.opsForValue().get("otp:"+normalized);
        if (stored==null || !stored.equals(otp)) throw new BusinessException(ErrorCode.AUTH_010_INVALID_OTP,HttpStatus.UNAUTHORIZED);
        redis.delete("otp:"+normalized); jdbc.update("UPDATE users SET is_email_verified=true WHERE email=?",normalized);
