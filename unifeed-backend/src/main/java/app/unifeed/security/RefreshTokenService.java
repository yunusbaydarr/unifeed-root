...
public class RefreshTokenService {
    ...
    public RefreshTokenService(StringRedisTemplate redis) { this.redis = redis; }
    ...
    private ResponseCookie cookie(String token) { return ResponseCookie.from("refreshToken", token).httpOnly(true).secure(true).sameSite("Strict").path("/api/v1/auth").maxAge(TTL).build(); }
